...
#'   \item{`"sync"`}{Every right-hand side is read from `s_t` (the state
#'     at the *start* of the call), so the signal advances exactly one
#'     edge per call. This is the natural mode for cyclic / dynamical
#'     fields --- in particular stock-and-flow consumption pipelines,
#'     which are inherently cyclic and cannot sweep.}
#' }
#'
...
#' See [runF()] to have the stock set derived automatically from the
#' action nodes, and `vignette("coffee_field_model", package =
...
#'   as `s_t`.
#'
#' @seealso [runF()] for the iteration driver (which derives `stocks`
#'   from the action set), [labelF()] for building the model string.
#'
#' @export
