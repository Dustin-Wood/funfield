...
section, every fragment colored **limegreen** is an FZ term (the part of
an arm that varies with `Z`); the leading F1 term stays black.

``` r

## Local helpers for the three moderation schematics below. Wrap a
## fragment in limegreen to flag it as an FZ coefficient, and in light
...
The three terms sum to (an approximation of) the green Term-0 arrow in
Figure 1.
...
- **Expectation panel:** `FZ[X,M](SRFast)` on the green left arm,
  `F1[M,Y]` on the black right arm. A green-black chain through a
...
- **Valuation panel:** `F1[X,M]` on the black left arm,
  `FZ[M,Y](SRFast)` on the green right arm. A black-green chain says
