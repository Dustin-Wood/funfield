...
```{r pathf-plot-moderated, fig.width = 9, fig.height = 4.5, out.width = "100%"}
plotPathF(dec_pf, Z_label = "SRFast",
          title = "The crash-injury cascade, moderated by fast-driver tendency")
```

The gold fragments locate the moderation visually: the working shifts
...
physical consequences once underway. A `Z_value` argument collapses
the same diagram to the effective field at a chosen level of the
moderator, and `Z_overlay = TRUE` renders the all-gold `FZ` field on
its own.
