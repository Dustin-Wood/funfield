...
`overtimeESJT$codebook$scenario$template`; the `${...}` placeholders
are filled in per respondent from the eight factors that follow:
...
`overtimeESJT$cond$scenario`. Three examples (rows 1-3):

```{r scenario-assembled}
overtimeESJT$cond$scenario[1:3]
```
